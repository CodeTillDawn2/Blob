...
public static class AIEditorBaker
{
    ...
    public static Dictionary<string, Dictionary<string, List<string>>> BakeCharacterSystemsToConfigMappings()
    {
        ...
        // New data structure: Outer dictionary with baseType's name as key, inner dictionary has derivedType's name as key and list of configuration types as value
        ...
        foreach (var baseType in baseTypes)
        {
            ...
            foreach (var derivedType in derivedTypesFromBase)
            {
                ...
                var configTypes = AppDomain.CurrentDomain.GetAssemblies()
                    .SelectMany(assembly => assembly.GetTypes())
                    .Where(type => type.IsSubclassOf(typeof(ConfigurationBase))
                            && requiredInterfaces.All(iface => iface.IsAssignableFrom(type)))
                    .Select(t => t.FullName)
                    .ToList();

                configTypes.Insert(0, "None"); // Adding the "None" option at the beginning

                innerDictionary[derivedType.FullName] = configTypes;

                LogToFile($"Populated {configTypes.Count} ConfigurationBase types for {derivedType.FullName} that implement all required interfaces.");
            }

            mappings[baseType.Name] = innerDictionary;  // Use the Name of the type instead of the type itself as the key
        }

        stopwatch.Stop();
        LogToFile($"PopulateDerivedTypeDictionary completed in: {stopwatch.Elapsed.TotalSeconds} seconds.");

        WriteToDisk(mappings, CharacterSystemToConfigMapping_Path);
        return mappings;
    }

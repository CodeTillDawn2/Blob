...
public class TentacleController : MonoBehaviour, IHaveTentacleReach, IHaveTentacleHitSpeed, IAmAlive
{

    public FloatVariable MinTentacleReach { get; set; }
    public FloatVariable MaxTentacleReach { get; set; }

    public FloatVariable TentacleHitSpeed { get; set; }
    ...
    {
        TentacleBrain tentacleBrain = brain.GetComponent<TentacleBrain>();
        tentacleBrain.target = target;
    ...
    private List<ImpulseStep> Relocate()
    {
    ...
    private List<ImpulseStep> Retarget()
    {
    ...
    public List<ImpulseStep> ShrinkToNothing()
    {

        //transform.localScale = Vector3.one * (1 - percentElapsed);
        return null;
    }
    public List<ImpulseStep> GrowToFullSize()
    {
    ...
    public bool CanReachTheTarget(Transform theTarget)
    {
        ...
        if (sqDistance > MaxTentacleReach.Value * MaxTentacleReach.Value
                || sqDistance < MinTentacleReach.Value * MinTentacleReach.Value)
        {
            return false;
        }
        return true;

...
public class BlobJiggleNew : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        for (int i = 0; i < jv.Length; i++)
        {
            ...
            jv[i].Position += new Vector3(.1f, .1f, .1f);

            target = transform.InverseTransformPoint(jv[i].Position);
            vertexArray[jv[i].ID] = Vector3.Lerp(vertexArray[jv[i].ID], target, 10);
        ...
        //    jv[i].Shake(target, Mass, stiffness, damping, minVelocity, maxVelocity);
        //    target = transform.InverseTransformPoint(jv[i].Position);
        //    vertexArray[jv[i].ID] = Vector3.Lerp(vertexArray[jv[i].ID], target, intensity);
        //}
        //MeshClone.vertices = vertexArray;
        //print("Top velocity: " + HighestVelocity);
    }

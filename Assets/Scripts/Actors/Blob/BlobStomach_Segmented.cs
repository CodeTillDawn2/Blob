...
public class BlobStomach_Segmented : BlobStomach
{
    ...
    private Vector3 SubCubeDims
    {
        get
        {
            return new Vector3(OriginalMeshSize.x * RigidBodyObject.transform.localScale.x,
                OriginalMeshSize.y * RigidBodyObject.transform.localScale.y,
                OriginalMeshSize.z * RigidBodyObject.transform.localScale.z);
        }
    }
    ...
    private GameObject[,,] SubBoxesLocations = new GameObject[10, 10, 10];
    ...
    private void AssignSubBoxes()
    {
        ...
        for (x = 0; x < 10; x++)
        {
            for (y = 0; y < 10; y++)
            {
                for (z = 0; z < 10; z++)
                {
                    GameObject subBox = GameObject.Find("Cube_" + (x + 1) + "_" + (y + 1) + "_" + (z + 1));
                    SubCubeInstructions.Add(subBox, new int[3] { x, y, z });
                    SubBoxesLocations[x, y, z] = subBox;
                    if (SubBoxesLocations[x, y, z] == null)
                    {
                        throw new Exception("Correctly named cubes not present in blob model");
                    }
                }
            }
        }
    ...
    public void MoveCubesToMatch()
    {
        ...
        for (int x = 0; x < 10; x++)
        {
            for (int y = 0; y < 10; y++)
            {
                for (int z = 0; z < 10; z++)
                {
                    GameObject isThere = SubBoxesLocations[x, y, z];
                    int[] instructions = SubCubeInstructions[isThere];

                    isThere.transform.position += new Vector3(instructions[0] - x, instructions[1] - y, instructions[3] - z) * Time.deltaTime;
                }
            }
        }
    }
    ...
    {
        for (int x = 0; x < 10; x++) //Bottom section slides back one 
        {
            for (int z = 0; z < 9; z++)
            {
                GameObject subcube = SubBoxesLocations[x, 0, z];
                SubCubeInstructions[subcube] = new int[3] { x, 0, z };
                SubBoxesLocations[x, 0, z].transform.position += new Vector3(0, 0, -SubCubeDims.z * Time.deltaTime);
            }
        }
        for (int x = 0; x < 10; x++) //Back slides up
        {
            for (int y = 0; y < 9; y++)
            {
                SubBoxesLocations[x, y, 9].transform.position += new Vector3(0, SubCubeDims.y * Time.deltaTime, 0);
            }
        }
    }
    ...
    protected override void ResetCubeDims()
    {
        ...
        BodyDims.Value.x = RigidBodyObject.transform.localScale.x * OriginalSize.Value.x;
        BodyDims.Value.y = RigidBodyObject.transform.localScale.y * OriginalSize.Value.y;
        BodyDims.Value.z = RigidBodyObject.transform.localScale.z * OriginalSize.Value.z;

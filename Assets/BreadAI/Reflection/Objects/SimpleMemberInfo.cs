...
public abstract class SimpleMemberInfo
{
    ...
    /// <summary>
    /// Gets or sets a dictionary of simplified attribute information attached to the member's interfaces.
    /// </summary>
    public Dictionary<Type, List<SimpleAttributeInfo>> InterfaceAttributes { get; set; } = new Dictionary<Type, List<SimpleAttributeInfo>>();
...
public List<SimpleAttributeInfo> ReflectAttributes()
{
    ...
    {
        attributes = me1.CachedPropertyInfo.GetCustomAttributes(true)
                         .OfType<Attribute>();
    }
    else if (this is SimpleMethodInfo me2)
    {
        attributes = me2.CachedMethodInfo.GetCustomAttributes(true)
                         .OfType<Attribute>();
    }
    else if (this is SimpleFieldInfo me3)
    {
        attributes = me3.CachedFieldInfo.GetCustomAttributes(true)
                         .OfType<Attribute>();
    }
    ...
    {
        return attributes
            .Where(x => x is BreadAIAttributeBase || x is BasicComponentAttribute) // Filtering attributes by specific types
